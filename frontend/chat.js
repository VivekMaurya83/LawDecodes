document.addEventListener("DOMContentLoaded", async () => {
  ...
  const dom = {
    ...
    pasteTextModal: document.getElementById("pasteTextModal"),
    closePasteModalBtn: document.getElementById("closePasteModalBtn"),
    submitPasteBtn: document.getElementById("submitPasteBtn"),
  ...
  // --- PASTE TEXT MODAL LOGIC ---
  dom.pasteTextBtn.addEventListener("click", () => {
    dom.pasteTextModal.style.display = "flex";
    // Use setTimeout to allow the display change to register before adding class
    setTimeout(() => {
        dom.pasteTextModal.classList.add("visible");
    }, 10);
  });

  function closeModal() {
    dom.pasteTextModal.classList.remove("visible");
    setTimeout(() => {
        dom.pasteTextModal.style.display = "none";
    }, 300); // Wait for transition to finish
  }

  dom.closePasteModalBtn.addEventListener("click", closeModal);
  dom.pasteTextModal.addEventListener("click", (e) => {
    // Close if user clicks on the dark overlay
    if (e.target === dom.pasteTextModal) {
        closeModal();
    }
  });

  dom.submitPasteBtn.addEventListener("click", async () => {
    const rawText = dom.pasteTextArea.value;
    if (!rawText.trim()) {
        alert("Please paste some text into the text area.");
        return;
    }

    dom.submitPasteBtn.textContent = "Processing...";
    dom.submitPasteBtn.disabled = true;

    try {
        const response = await authorizedFetch(`${API_BASE_URL}/api/upload-text`, {
            method: "POST",
            body: JSON.stringify({ raw_text: rawText }),
        });

        if (!response.ok) throw new Error("Failed to process pasted text.");
        
        const result = await response.json();
        
        // RE-USE the existing function to add the file message to the chat
        addFileMessageToChat(result.filePath, result.originalFilename);
        await saveCurrentChatToBackend();
        
        closeModal(); // Close the modal on success
        dom.pasteTextArea.value = ""; // Clear the text area
        
    } catch (error) {
         if (error.message !== "Session expired") {
            console.error("Error processing pasted text:", error);
            alert("Could not process your text.");
         }
    } finally {
        dom.submitPasteBtn.textContent = "Analyze Pasted Text";
        dom.submitPasteBtn.disabled = false;
    }
  });
  ...
  dom.chatWindow.addEventListener("click", async (e) => {
    ...
    if (e.target.classList.contains("compare-btn")) {
      ...
      // This selector correctly finds the div holding the rendered summary
      const summaryContentElement = button.closest(".message-content").querySelector("div");
      
      if (!summaryContentElement) {
          alert("Error: Could not find summary content to display.");
          button.textContent = "Compare Side-by-Side";
          button.disabled = false;
          return;
      }
      
      const summaryText = summaryContentElement.innerHTML;
      ...
      try {
        ...
        // FIXED: Use .textContent for plain text and .innerHTML for HTML content
        dom.originalTextContent.textContent = data.original_text;
